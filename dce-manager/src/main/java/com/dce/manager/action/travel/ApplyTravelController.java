...
package com.dce.manager.action.travel;
...
public class ApplyTravelController extends BaseAction{
	@Resource
	private ITravelApplyService applyTravelService;
    ...
    public void listApplyTravel(NewPagination<TravelDo> pagination,
        ...
        try{
            ...
            page = applyTravelService.getApplyTravelPage(param, page);
    ...
    public String addApplyTravel(String id, ModelMap modelMap, HttpServletResponse response) {
        ...
        try{
            if(StringUtils.isNotBlank(id)){
                TravelDo applytravelDo = applyTravelService.getById(Long.valueOf(id));
                if(null != applytravelDo){
                    modelMap.addAttribute("applytravel", applytravelDo);
                }
            }
    ...
    @ResponseBody
    public void saveApplyTravel(TravelDo applytravelDo, 
    							  HttpServletRequest request, 
    							  HttpServletResponse response) {
        logger.info("----saveApplyTravel------");
        try{
            Integer id = applytravelDo.getId();
            Long userId = new Long(this.getUserId());
            
            int i = 0;
            if (id != null && id.intValue()>0) {
                i = applyTravelService.updateApplyTravelById(applytravelDo);
            } else {
                applyTravelService.travelApply(applytravelDo);
            }
    ...
    public void export(HttpServletResponse response) throws IOException {
        try {
            ...
            ApplyTravelExample example  = new ApplyTravelExample();
            String companyName = getString("searchPolicyName");
            ...
            List<TravelDo> applytravelLst = applyTravelService.selectApplyTravel(example);
