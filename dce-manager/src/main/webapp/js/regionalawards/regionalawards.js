...
/*#############################################search form begin#################################*/	
	...
	$("#searchregionalawardsForm #searchButton").on("click",function(){
		$("#tt_Regionalawards").datagrid('load',{
			'searchStr': $("#searchregionalawardsForm #searchStr").val(),
			'searchCodeStr':$("#searchregionalawardsForm #searchCodeStr").val()		
		});
	});
	...
	var columns_tt = [
      			[	 				
							{field:'id',title:'id',width:100,hidden:true},						
								{field:"rewardBalance",title:"编码",width:180,align:"center"},
								{field:" algebra",title:"编码",width:180,align:"center"},
					{field:"操作",title:"操作",width:80,align:"left",
	 					formatter:function(value,row,index){
	 					  var str= '<a href="javascript:void(0);" onclick="to_editregionalawards(\''+row.id+'\');">编辑</a>';
	 					  return str;
	 					}
	 				}	 				
	 			]
...
/*######################grid columns end##############################*/
	...
	$("#tt_Regionalawards").datagrid({
		url:httpUrl+"/regionalawards/listRegionalawards.html?&rand=" + Math.random(),
		height:$("#body").height()-$('#search_areaRegionalawards').height()-10,
		width:$("#body").width(),
		rownumbers:true,
		fitColumns:true,
...
function to_editregionalawards(id){
	...
	var url = httpUrl+"/regionalawards/addRegionalawards.html?&rand=" + Math.random()+"&id="+id;
...
function save_Regionalawards(){
	...
	console.info(formdata);
	var  url =httpUrl+"/regionalawards/saveRegionalawards.html?&rand=" + Math.random();
	 $.ajax({   
		 type: 'POST',
		 dataType: 'json',
		 url: url,  
...
//改变表格和查询表单宽高
function domresize(){
	$('tt_Regionalawards').datagrid('resize',{  
		height:$("#body").height()-$('#search_areaRegionalawards').height()-5,
		width:$("#body").width()
	});
