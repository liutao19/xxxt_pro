...
//	 				{field:"balanceIntegral",title:"原持仓",width:80,align:"center"},
	 				...
	 				{field:"edit",title:"操作",width:80,align:"center",
	 					formatter:function(value,row,index){
	 					  var href = "";
	 					  if(row.status == '0'){
	 						  href = href + '<a href="javascript:void(0);"  onclick="to_lock('+row.id+',\''+1+'\');">冻结账户</a>';
	 					  }else if(row.status == '1'){
		 					  
	 						  href = href + '<a href="javascript:void(0);"  onclick="to_lock('+row.id+',\''+0+'\');">解冻账户</a>';
	 					  }
	 					  
	 					  if(row.isActivated != undefined && row.isActivated != 1){
	 						  href = href + ' <a href="javascript:void(0);"  onclick="baoKongDan('+row.id+');">激活</a>';
	 					  }
