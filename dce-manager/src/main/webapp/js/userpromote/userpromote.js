	...
	var columns_tt = [
      			[	 				
							{field:'id',title:'id',width:100,hidden:true},						
								{field:"userLevel",title:"编码",width:180,align:"center"},
								{field:"promoteLevel",title:"编码",width:180,align:"center"},
								{field:"minQty",title:"编码",width:180,align:"center"},
								{field:"maxQty",title:"编码",width:180,align:"center"},
					{field:"操作",title:"操作",width:80,align:"left",
	 					formatter:function(value,row,index){
	 					  var str= '<a href="javascript:void(0);" onclick="to_edituserPromote(\''+row.id+'\');">编辑</a>';
	 					  return str;
	 					}
	 				}	 				
	 			]
...
/*######################grid columns end##############################*/
	...
	$("#tt_UserPromote").datagrid({
		url:httpUrl+"/userpromote/listUserPromote.html?&rand=" + Math.random(),
		height:$("#body").height()-$('#search_areaUserPromote').height()-10,
		width:$("#body").width(),
		rownumbers:true,
		fitColumns:true,
...
function to_edituserPromote(id){
	...
	var url = httpUrl+"/userpromote/addUserPromote.html?&rand=" + Math.random()+"&id="+id;
...
function save_UserPromote(){
	...
	console.info(formdata);
	var  url =httpUrl+"/userpromote/saveUserPromote.html?&rand=" + Math.random();
	 $.ajax({   
		 type: 'POST',
		 dataType: 'json',
		 url: url,  
...
//改变表格和查询表单宽高
function domresize(){
	$('tt_UserPromote').datagrid('resize',{  
		height:$("#body").height()-$('#search_areaUserPromote').height()-5,
		width:$("#body").width()
	});
