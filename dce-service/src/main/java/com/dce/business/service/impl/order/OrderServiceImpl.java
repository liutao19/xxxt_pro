...
public class OrderServiceImpl implements IOrderService {
	...
	private Double countPremiumPriceSpread(List<OrderDetail> premiumList) {
		...
		if (premiumList.size() <= 1) {
			if (premiumList.get(0).getOrderid() == 1) { // 男版
				totalprice = 0;
			} else { // 女版差价
	...
	public Result<String> getAlipayorderStr(Order order) {
		...
		alipaymentOrder.setCreatetime(new Date());
		...
		try {
			AlipayClient alipayClient = new DefaultAlipayClient(AlipayConfig.URL, AlipayConfig.APPID,
					AlipayConfig.RSA_PRIVATE_KEY, AlipayConfig.CHARSET,
