...
public class OrderServiceImpl implements IOrderService {
	...
	public Result<?> buyOrder(Integer userId, Long orderId, BigDecimal qty) {
		...
		if (ctCurrencyDo != null && ctCurrencyDo.getCurrency_sell_fee() != null) {
			BigDecimal feeRate = ctCurrencyDo.getCurrency_sell_fee().divide(new BigDecimal("100"), 6,
	...
	public Result<?> guadan(OrderDo orderDo) {
		...
		if (orderDo.getPrice().compareTo(ct.getPrice_down()) < 0
				|| orderDo.getPrice().compareTo(ct.getPrice_up()) > 0) {
	...
	private Result<?> guadanSal(OrderDo orderDo) {
		...
		if (currentAcc == null || currentAcc.getAmount() == null
				|| currentAcc.getAmount().compareTo(orderDo.getQty()) < 0) {
	...
	public Result<?> cancel(Long orderId, Integer userId) {
		...
		/*
		 * int flag = updateOrder(_ord); if(flag > 0){ return
		 * Result.successResult("订单撤销成功"); }else{
		 * 
